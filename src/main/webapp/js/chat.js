...
chat.ChatModel.prototype = {
	...
	login: function(nickName) {
		...
		new ajax.xhr.Request("/AJAX_chat_lec/jsp/login.jsp", params,
	...
	logout: function() {
		...
		new ajax.xhr.Request("/AJAX_chat_lec/jsp/logout.jsp", params,
	...
	loadMessage: function() {
		...
		new ajax.xhr.Request("/AJAX_chat_lec/jsp/loadMessage.jsp", params,
		...
		if (req.readyState == 4) {
			if (req.status == 200) {
				...
				if (code == 'success') {
					...
					this.notify(chat.MESSAGE_LOADED_ERROR, "MESSAGE_LOAD_ERROR");
				}
			} else {
	...
	sendMessage: function(msg,click) {
	//
		var params = "nickName=" + encodeURIComponent(this.nickName) +
			"&msg=" + encodeURIComponent(msg)+"&click="+encodeURIComponent(click);
		new ajax.xhr.Request("/AJAX_chat_lec/jsp/sendMessage.jsp", params,
...
chat.Chat = function() {
	...
	// Model에 Controller를 등록
	this.chatModel.addListener(this);
	// View에 Controller를 등록
...
chat.Chat.prototype = {
	...
	sendMessage: function(message,click) {
		this.chatModel.sendMessage(message,click);
		console.log(click);
	}
}
...
chat.ChatUI.prototype = {
	...
	makeChatWindow: function() {
		...
		var ipBoxInput = document.createElement("input");
		ipBoxInput.type = "checkbox";
		...
		chatWinDiv.appendChild(ipBoxInput);
	...
	doKeyDownOnInput: function(e) {
		...
		if (e.keyCode == 13) { //
			...
			var click = this.ipBoxInput.checked;
			this.messageInput.value = "";
			this.chat.sendMessage(msg,click);
		}
	},
	...
	processError: function(type, errMsg) {
		if (type == chat.LOGIN_ERROR) {
			alert("LOGIN_ERROR :" + errMsg);
		} else if (type == chat.MESSAGE_LOADED_ERROR) {
			alert("MESSAGE_LOADED_ERROR :" + errMsg);
		} else if (type == chat.MESSAGE_SENDED_SUCCESS) {

		} else if (type == chat.MESSAGE_SENDED_ERROR) {
			alert("MESSAGE_SENDED_ERROR :" + errMsg);
		} else if (type == chat.LOGOUT_ERROR) {
			alert("LOGOUT_ERROR :" + errMsg);
		}
	}
}